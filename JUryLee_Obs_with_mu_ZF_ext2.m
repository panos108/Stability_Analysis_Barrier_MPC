...
% mini = 4.
%%%%%%%%%%% for b = 0.5 ----> r = 0.724%%%%%%%%
%%%%%%%%%%% for r = 0.1 ----> b = 0.314^2=0.0986 %%%%%%%
%%%%%%%%%%% for n = 10    ----> b = 0.710 %%%%the most I can get
for par1 =0.1;%.724:-0.001:0.0001
        k=par1;
    pp1=pp1+1;
    pp2=0;
    for par2 = 1.09:0.001:10
        pp2=pp2+1;
        %%% Define system %%%
        z=tf('z');
        %%% Gain for plant %%%
        ...
        for i = 1:2*n
            KK{i} = sdpvar*eye(ns);% diag(sdpvar(ns,1));
        end
...
%         K2 = diag(sdpvar(ns,1));
%         K1 = diag(sdpvar(ns,1));
         Ko =  sdpvar*eye(ns);%diag(sdpvar(ns,1));
        % %
...
%         
        ...
        if max(eig(value(X'*GGG*X)))>0||p1.problem~=0
            results1(pp1,1)=4;
            break
        else
            results1(pp1,1)=0;
